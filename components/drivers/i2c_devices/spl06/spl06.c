...
#define DEBUG_MODULE "SPL06"
#include "debug_cf.h"

#define P_MEASURE_RATE 			SPL06_MWASURE_16 	// measurements per second
#define P_OVERSAMP_RATE 		SPL06_OVERSAMP_64	// oversampling rate
#define SPL06_PRESSURE_CFG		(P_MEASURE_RATE<<4 | P_OVERSAMP_RATE)

#define T_MEASURE_RATE 			SPL06_MWASURE_16 	// measurements per second
#define T_OVERSAMP_RATE 		SPL06_OVERSAMP_8	// oversampling rate
...
static spl06CalibCoefficient_t spl06Calib;
...
static int32_t kp = 0;
static int32_t kt = 0;
static int32_t SPL06RawPressure = 0;
static int32_t SPL06RawTemperature = 0;

static void SPL06GetPressure(void);
static void spl0601_get_calib_param(void);
static void spl0601_rateset(spl06Sensor_e sensor, uint8_t measureRate, uint8_t oversamplRate);
...
bool SPL06Init(I2C_Dev *i2cPort)
{
	...
	i2cdevReadByte(I2Cx, devAddr, SPL06_CHIP_ID, &SPL06ID);	/* read the SPL06 ID */

	if(SPL06ID == SPL06_DEFAULT_CHIP_ID)
		DEBUG_PRINTI("SPL06 ID IS: 0x%X\n",SPL06ID);
    else
        return false;
...
//	pressureFilter(&p,pressure);
	*temperature = (float)t;	/* degrees Celsius */
	*pressure = (float)p;	/* hPa */
	
	*asl = SPL06PressureToAltitude(*pressure);	/* metres above sea level */
	return 1;
}
...
bool SPL06SelfTest(void)
{
    /* The chip ID was already verified in SPL06Init(), and the calibration
     * coefficients are only plausible once the part answers, so there is
