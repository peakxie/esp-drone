...
def main():
    ...
    if results:
        ...
        print(
            ...
            "2. 直接看 vx/vy 峰值：机体坐标系是 FLU（X=前，Y=左，Z=上）。朝前推 vx 应为正、"
            "朝后为负；朝左推 vy 应为正、朝右为负。\n"
            "   如果峰值符号和上面反过来、或者悬停不推它自己也会朝某个方向持续报出非零速度"
            "（虚假速度），说明光流方向/融合有问题，去改 flowdeck_v1v2.c 里 accpx/accpy 那两行。\n"
            "   （2025 年这一轮实测：朝后两次都测出 vx 为正、朝左两次都测出 vy 为负，跟 FLU 约定"
            "正好反过来，已经把 flowdeck_v1v2.c 里的 dpixelx=-deltaY,dpixely=-deltaX 改成不取负的"
            "dpixelx=deltaY, dpixely=deltaX——如果你在改动之后又跑出同样反号的结果，说明改错了"
            "或者又有别的问题，不要想当然。）"
        )
